...
const HeroOptions = computed(() => {
  const heroes = tokenStore.gameData?.roleInfo?.role?.heroes;
  if (!heroes) return [];
  return Object.values(heroes).map((item) => {
    return {
      label: HERO_DICT[item.heroId]?.name + "(" + item.level + "/6000)" || `英雄${item.heroId}(${item.level}/6000)`,
      value: item.heroId,
...
watch(
  ...
  () => {
    const heroes = tokenStore.gameData?.roleInfo?.role?.heroes;
    if (HeroValue.value && heroes) {
      if (heroes[HeroValue.value]?.level != 6000) {
        HeroItem.value = Object.assign(
          {},
          heroes[HeroValue.value],
