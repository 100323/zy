    ...
    <div class="status-card main-card">
      ...
      <div class="debug-panel">
        <div class="debug-header">
          <span>调试信息</span>
          <span class="debug-meta">连接: {{ debugSnapshot.wsStatus || '-' }} / battleVersion: {{ debugSnapshot.battleVersion || '-' }}</span>
        </div>
        <div class="debug-list">
          <div v-if="debugLogs.length === 0" class="debug-empty">暂无调试日志</div>
          <div v-for="(log, index) in debugLogs" :key="index" class="debug-item" :class="`debug-${log.type}`">
            <div class="debug-item-head">{{ log.time }} · {{ log.step }}</div>
            <pre class="debug-item-body">{{ log.detail }}</pre>
          </div>
        </div>
      </div>
...
const pushDebugLog = (step, detail, type = 'info') => {
  ...
  debugLogs.value = debugLogs.value.slice(0, 12);
};
...
const refreshDebugSnapshot = (tokenId) => {
  debugSnapshot.value = {
    wsStatus: tokenStore.getWebSocketStatus(tokenId),
    battleVersion: tokenStore.getBattleVersion(),
  };
};
...
const fetchfightPVP = async () => {
  ...
  const numericTargetId = Number(targetId.value);
  const resolvedTargetId = Number(memberData.value?.roleId || numericTargetId);
  resetDebugLogs();
  refreshDebugSnapshot(tokenId);
  pushDebugLog("切磋开始", { tokenId, targetId: targetId.value, numericTargetId, resolvedTargetId, fightNum: fightNum.value, snapshot: debugSnapshot.value });

  // 检查WebSocket连接
  const wsStatus = tokenStore.getWebSocketStatus(tokenId);
  if (wsStatus !== "connected") {
    pushDebugLog("连接检查失败", { wsStatus }, "error");
    message.error("WebSocket未连接，无法查询战绩");
    return;
  }

  if (!Number.isFinite(resolvedTargetId) || resolvedTargetId <= 0) {
    pushDebugLog("目标ID无效", { targetId: targetId.value, numericTargetId, resolvedTargetId }, "error");
    message.error("目标ID无效，请输入纯数字ID");
    return;
  }
  ...
  try {
    ...
    for (var i = 0; i < fightNum.value; i++) {
      try {
        pushDebugLog(`切磋预热 ${i + 1}/${fightNum.value}`, { step: 'role_getroleinfo' });
        await tokenStore.sendMessageWithPromise(tokenId, "role_getroleinfo", {}, 5000);
      } catch (preheatError) {
        pushDebugLog('切磋预热 role_getroleinfo 失败', { message: preheatError?.message }, 'error');
      }

      try {
        pushDebugLog(`切磋预热 ${i + 1}/${fightNum.value}`, { step: 'fight_startlevel' });
        const levelInfo = await tokenStore.sendMessageWithPromise(tokenId, "fight_startlevel", {}, 5000);
        const latestBattleVersion = Number(levelInfo?.battleData?.version || levelInfo?.battleVersion || 0);
        pushDebugLog('切磋预热 fight_startlevel 返回', { latestBattleVersion, keys: levelInfo ? Object.keys(levelInfo) : [] });
      } catch (preheatError) {
        pushDebugLog('切磋预热 fight_startlevel 失败', { message: preheatError?.message }, 'error');
      }

      pushDebugLog(`发送切磋请求 ${i + 1}/${fightNum.value}`, { cmd: "fight_startpvp", params: { targetId: resolvedTargetId }, battleVersion: tokenStore.getBattleVersion(), memberRoleId: memberData.value?.roleId });
      ...
      pushDebugLog(`收到切磋响应 ${i + 1}/${fightNum.value}`, { hasBattleData: !!result?.battleData, keys: result ? Object.keys(result) : [] });
      if (!result.battleData) {
        fightResult.value = null;
        pushDebugLog("切磋响应异常", result || "无返回内容", "error");
    ...
    pushDebugLog("切磋失败", { message: error?.message, stack: error?.stack }, "error");
    message.error(`切磋失败: ${error.message}`);
    fightResult.value = null;
  } finally {
    refreshDebugSnapshot(tokenId);
...
const fetchTargetInfo = async () => {
  ...
  resetDebugLogs();
  refreshDebugSnapshot(tokenId);
  pushDebugLog("查询对手开始", { tokenId, targetId: targetId.value, numericTargetId, snapshot: debugSnapshot.value });
  ...
  if (!Number.isFinite(numericTargetId) || numericTargetId <= 0) {
    pushDebugLog("目标ID无效", { targetId: targetId.value, numericTargetId }, "error");
    message.error("目标ID无效，请输入纯数字ID");
    return;
  }
  ...
  try {
    pushDebugLog("发送对手查询", { cmd: "rank_getroleinfo", params: { roleId: numericTargetId } });
    const result = await tokenStore.sendMessageWithPromise(
      tokenId,
      "rank_getroleinfo",
      {
        ...
        isSearch: false,
        roleId: numericTargetId,
      },
      5000,
    );

    pushDebugLog("收到对手查询响应", { hasRoleInfo: !!result?.roleInfo, hasLegionInfo: !!result?.legionInfo, keys: result ? Object.keys(result) : [], roleId: result?.roleInfo?.roleId });
    if (!result.roleInfo && !result.legionInfo) {
      memberData.value = null;
      pushDebugLog("对手查询为空", result || "无返回内容", "error");
    ...
    // 俱乐部名称
    teamData.roleId = Number(result.roleInfo.roleId || numericTargetId || 0);
    ...
    pushDebugLog("查询对手失败", { message: error?.message, stack: error?.stack }, "error");
