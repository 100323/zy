...
const clubName = computed(() => club.value?.name || '俱乐部');
...
const ensureWsConnected = async (tokenId) => {
  let wsStatus = tokenStore.getWebSocketStatus(tokenId)
  if (wsStatus === 'connected') return true

  const token = tokenStore.selectedToken
  pushDebugLog('检测到连接断开，尝试自动重连', { tokenId, wsStatus })
  ...
  while (Date.now() - startedAt < 6000) {
    ...
    if (wsStatus === 'connected') {
      ...
      pushDebugLog('自动重连成功', { tokenId, wsStatus: 'connected' }, 'success')
      return true
    }
  ...
  refreshDebugSnapshot(tokenId)
  pushDebugLog('自动重连失败', { tokenId, wsStatus: tokenStore.getWebSocketStatus(tokenId) }, 'error')
  return false
}
...
// 查询战绩
  const fetchBattleRecords = async () => {
    ...
    resetDebugLogs()
    refreshDebugSnapshot(tokenId)
    pushDebugLog('战绩查询开始', { tokenId, queryDate: queryDate.value, snapshot: debugSnapshot.value })

    // 检查WebSocket连接
    const wsStatus = tokenStore.getWebSocketStatus(tokenId)
    if (wsStatus !== 'connected') {
      pushDebugLog('连接检查失败', { wsStatus }, 'error')
      const reconnected = await ensureWsConnected(tokenId)
      if (!reconnected) {
        message.error('WebSocket未连接，且自动重连失败，无法查询战绩')
        return
      }
    }
    ...
    try {
      pushDebugLog('发送战绩请求', { cmd: 'legionwar_getdetails', params: { date: queryDate.value }, battleVersion: tokenStore.getBattleVersion(), timeoutMs: 10000 })
      const result = await Promise.race([
        ...
        new Promise((_, reject) => {
          setTimeout(() => reject(new Error('页面级超时: legionwar_getdetails 超过 12 秒未返回')), 12000)
        }),
      ])

      pushDebugLog('收到战绩响应', { hasRoleDetailsList: !!result?.roleDetailsList, count: result?.roleDetailsList?.length || 0, keys: result ? Object.keys(result) : [] })
      if (result && result.roleDetailsList) {
        ...
        pushDebugLog('战绩数据为空', result || '无返回内容', 'error')
        message.warning('未查询到战绩数据')
      }
    } catch (error) {
      console.error('查询战绩失败:', error)
      pushDebugLog('查询战绩失败', { message: error?.message, stack: error?.stack }, 'error')
