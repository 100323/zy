...
export const useTokenStore = defineStore("tokens", () => {
  ...
  const syncAccountToBackend = async (tokenData: any) => {
    ...
    const syncPromise = (async () => {
      ...
      try {
        ...
        await api.put(`/accounts/${exists.id}`, {
          token: rawToken,
          server: tokenData.server || "",
          wsUrl: tokenData.wsUrl || "",
          remark: tokenData.remark || "",
    ...
    backendHydrationPromise = (async () => {
      try {
        const pendingLocalTokens = gameTokens.value.filter((token) => {
          return !/^\d+$/.test(String(token.id)) && !!String(token.token || "").trim();
        });
        ...
        const backendTokens = await Promise.all(
          backendAccounts.map(async (account: any) => {
            ...
            if (!rawToken) {
              try {
                const tokenRes = await api.get(`/accounts/${accountId}/token`);
                if (tokenRes?.success) {
  ...
  const removeToken = async (tokenId: string) => {
    const isBackendAccountId = /^\d+$/.test(String(tokenId));
    if (isBackendAccountId) {
      try {
        await api.delete(`/accounts/${tokenId}`);
      } catch (error) {
        tokenLogger.warn(`删除后端账号失败 [${tokenId}]，继续清理本地`, error);
      }
    }
  ...
  const getClientVersion = () => {
    if (typeof globalThis !== "undefined") {
      const runtimeVersion = globalThis.GAME_VERSION || globalThis.CODE_VERSION;
  ...
  const isLegacyWsUrl = (wsUrl?: string | null) => {
    if (!wsUrl || !wsUrl.includes("?p=")) return false;
    try {
      ...
      return !wsUrl.includes("%7B") && !wsUrl.includes("{");
    }
  };

  const buildDefaultWsUrl = (actualToken: string) => {
    const now = Date.now();
    const sessionId = now * 100 + Math.floor(Math.random() * 100);
    const connId = now + Math.floor(Math.random() * 10);
    let payload = actualToken;
    ...
    try {
      ...
      if (parsed && typeof parsed === "object" && parsed.roleToken) {
        payload = JSON.stringify({
          ...
          sessId: sessionId,
          connId,
          version: parsed.version || getClientVersion(),
          isRestore: 0,
        });
      } else {
        throw new Error("not_conn_param");
      }
    } catch {
      payload = JSON.stringify({
    ...
    return `wss://xxz-xyzw.hortorgames.com/agent?p=${encodeURIComponent(payload)}&e=x&lang=chinese`;
  };
  ...
  const importBase64Token = (
    ...
    try {
      ...
      return {
        success: false,
        error: errorMessage,
        message: `Token "${name}" 添加失败: ${errorMessage}`,
      };
    }
  };
  ...
  const updateCrossTabConnectionState = (
    tokenId: string,
    action: CrossTabConnectionState["action"],
    sessionId: string = currentSessionId,
  ) => {
    const state = useLocalStorage<CrossTabConnectionState>(`ws_connection_${tokenId}`, {
      action, // 'connecting', 'connected', 'disconnecting', 'disconnected'
      sessionId,
      timestamp: Date.now(),
  ...
  const checkCrossTabConnection = (tokenId: string) => {
    ...
    try {
      ...
      if (stored) {
        const state = JSON.parse(stored) as CrossTabConnectionState;
  ...
  const createWebSocketConnection = async (
    ...
    try {
      ...
      // 6. 构建WebSocket URL
      const baseWsUrl = buildDefaultWsUrl(actualToken);
      ...
      wsClient.onConnect = () => {
        ...
        // 连接刚建立时按旧版客户端的节奏做一次初始化，避免后续玩法被服务端判定为版本状态未就绪。
        setTimeout(() => {
          try {
  ...
  const sendMessageWithPromise = async (
    ...
    let connection: Partial<WebSocketConnection> | null | undefined = wsConnections.value[tokenId];
    ...
    // 仅为明确依赖 battleVersion 的战斗命令自动注入版本。
    // 切磋/竞技场按原始协议只发 targetId，强行带版本会触发“版本过低”。
    ...
    try {
      ...
      if (shouldRetryWithBattleVersion) {
        ...
        wsLogger.warn(`检测到版本校验失败，使用 battleVersion 重试 [${tokenId}] ${cmd}:`, retryParams);
