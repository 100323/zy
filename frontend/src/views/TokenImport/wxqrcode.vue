...
import { Scan, Refresh, Close } from "@vicons/ionicons5";
...
const tryGetWeixinQR = async () => {
  try {
    ...
    updateStatus("二维码获取失败：" + err.message + "。建议使用其他导入方式", "error");
    return false;
  }
};
  ...
  try {
    ...
    if (res.status === 200) {
      ...
      if (text.includes("window.wx_errcode=405")) {
        // 优先从 wx_redirecturl 中完整提取 code，避免因字符集截断导致 invalid code
        const redirectMatch = text.match(/wx_redirecturl='([^']+)'/);
        let code = "";
        if (redirectMatch?.[1]) {
          ...
          try {
            ...
            const looseMatch = redirectUrl.match(/[?&]code=([^&]+)/);
            code = looseMatch?.[1] || "";
          }
        }
        if (!code) {
...
const ensureGameEncryptModule = async () => {
  ...
  if (!encryptionModuleLoadPromise) {
    encryptionModuleLoadPromise = (async () => {
      ...
      // 兜底：部分运行时只读取全局变量名，不走 globalThis 属性
      const g = globalThis as any;
      if (typeof g.PLATFORM === "undefined") g.PLATFORM = "wx";
      if (typeof g.SUB_PLATFORM === "undefined") g.SUB_PLATFORM = "";
      if (typeof g.ENV === "undefined") g.ENV = "Prod";
