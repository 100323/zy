...
const createDefaultBatchSettings = () => ({
...
const accountSettingsMap = ref({});
const currentSettings = reactive(createDefaultBatchSettings());
...
const currentTemplate = reactive(createDefaultBatchSettings());
...
const groupedAvailableTasks = computed(() => {
  ...
  taskGroupDefinitions.forEach((group) => {
    groups[group.name] = availableTasks.filter(
      (task) => task.group === group.name
    );
  });

  const groupedTaskValues = taskGroupDefinitions.flatMap((g) =>
    availableTasks.filter((t) => t.group === g.name).map((t) => t.value)
  );
...
const manualExecuteTask = async (task) => {
  ...
  try {
    ...
    const result = await api.batchScheduler.execute(task.id);
    if (result.success) {
      message.success(`任务 ${task.name} 已提交执行`);
      addLog({
        time: new Date().toLocaleTimeString(),
        message: `=== 定时任务 ${task.name} 已提交到后端执行 ===`,
        type: "info",
      });
    } else {
...
const loadScheduledTasks = async () => {
  try {
    const result = await api.batchScheduler.getList();
    if (result.success) {
      scheduledTasks.value = result.data.map(task => ({
        id: task.id,
        name: task.name,
        runType: task.run_type,
...
const saveTask = async () => {
  ...
  try {
    ...
    if (editingTask.value) {
      result = await api.batchScheduler.update(editingTask.value.id, taskData);
    } else {
      result = await api.batchScheduler.create(taskData);
    }
    ...
    if (result.success) {
      ...
      addLog({
        time: new Date().toLocaleTimeString(),
        message: `=== 定时任务 ${taskData.name} 已${editingTask.value ? '更新' : '创建'} ===`,
        type: "info",
      });
    } else {
...
const deleteTask = async (taskId) => {
  ...
  try {
    const result = await api.batchScheduler.delete(taskId);
    if (result.success) {
...
const toggleTaskEnabled = async (taskId, enabled) => {
  ...
  try {
    const result = await api.batchScheduler.update(taskId, { enabled });
    if (result.success) {
...
const exportConfig = () => {
  try {
    ...
    // Gather token settings from backend-backed cache
    const tokenSettings = tokens.value
      .map((token) => {
        const record = accountSettingsMap.value[String(token.id)];
        if (!record) {
          return null;
        }
        return {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (Array.isArray(importData.tokenSettings)) {
          for (const item of importData.tokenSettings) {
            const matchedToken = tokens.value.find((token) => {
              return String(token.id) === String(item.tokenId)
                || (item.tokenName && token.name === item.tokenName);
            });
...
const loadAllAccountSettings = async () => {
  try {
    const res = await api.batchSettings.getAccountSettings();
    if (!res?.success || !Array.isArray(res.data)) {
      return accountSettingsMap.value;
    }

    const nextMap = {};
    res.data.forEach((item) => {
      nextMap[String(item.accountId)] = {
        templateId: item.templateId != null ? String(item.templateId) : null,
        settings: normalizeBatchSettings(item.settings),
        updatedAt: item.updatedAt,
      };
    });
    accountSettingsMap.value = nextMap;
    return nextMap;
  } catch (error) {
    console.error("Failed to load account batch settings:", error);
    return accountSettingsMap.value;
  }
};

const loadSettings = (tokenId) => {
  const saved = accountSettingsMap.value[String(tokenId)];
  return normalizeBatchSettings(saved?.settings || {});
};
...
const openTaskTemplateModal = async () => {
  ...
  // 重置当前模板
  Object.assign(currentTemplate, createDefaultBatchSettings());
...
const loadTaskTemplates = async () => {
  try {
    const res = await api.batchSettings.getTemplates();
    taskTemplates.value = res?.success && Array.isArray(res.data) ? res.data : [];
  } catch (error) {
...
const applyTemplate = async () => {
  ...
  const template = taskTemplates.value.find((t) => String(t.id) === String(selectedTemplateId.value));
  if (!template) {
    message.error("模板不存在");
    return;
  }
...
const updateTaskTemplate = async () => {
  ...
  if (res?.success) {
    await loadTaskTemplates();
    message.success(`已更新模板 "${res.data.name}"`);
...
const deleteTaskTemplate = async (templateId) => {
  if (confirm("确定要删除这个模板吗？")) {
    const res = await api.batchSettings.deleteTemplate(templateId);
    if (res?.success) {
      await loadTaskTemplates();
...
const openAccountTemplateModal = async () => {
  await Promise.all([loadTaskTemplates(), loadAllAccountSettings()]);
  loadAccountTemplateReferences();
...
const loadAccountTemplateReferences = () => {
  ...
  sortedTokens.value.forEach((token) => {
    const settingsRecord = accountSettingsMap.value[String(token.id)];
    const templateId = settingsRecord?.templateId || null;
...
const saveTaskTemplate = async () => {
  ...
  if (currentTemplateId.value) {
    ...
    if (res?.success) {
      await loadTaskTemplates();
      message.success(`已保存模板 "${res.data.name}"`);
      showTaskTemplateModal.value = false;
      resetTemplateForm();
    }
  }
...
const { batcharenafight } = tasksArena;
...
const { batchLegacyClaim } = tasksLegacy;
