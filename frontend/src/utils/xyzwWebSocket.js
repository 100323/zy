...
export class XyzwWebSocketClient {
  ...
  init() {
    ...
    this.socket.onclose = (evt) => {
      ...
      this.connected = false;
      this._clearTimers();
      this._rejectPendingPromises(`WebSocket 连接关闭: ${evt.code}${evt.reason ? ` ${evt.reason}` : ""}`);
    ...
    this.socket.onerror = (error) => {
      ...
      this.connected = false;
      this._clearTimers();
      this._rejectPendingPromises("WebSocket 发生错误，挂起请求已取消");
  ...
  sendWithPromise(cmd, params = {}, timeoutMs = 5000) {
    return new Promise((resolve, reject) => {
      ...
      // 设置 Promise 状态，使用seq作为键，便于响应/断线时统一清理
  ...
  /** 断线时立即拒绝所有挂起请求，避免页面长时间卡在 loading */
  _rejectPendingPromises(reason = "WebSocket 连接已关闭") {
    for (const [requestId, promiseData] of Object.entries(this.promises)) {
      if (promiseData?.timer) clearTimeout(promiseData.timer);
      try {
        promiseData?.reject?.(new Error(reason));
      } catch (error) {
        wsLogger.warn("拒绝挂起请求失败:", error);
      }
      delete this.promises[requestId];
    }
  }
