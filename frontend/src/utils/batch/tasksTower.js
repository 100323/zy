  ...
  const climbTower = async () => {
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        const weirdTowerFormation = tokenSettings.weirdTowerFormation ?? tokenSettings.towerFormation ?? 1;
        const currentFormation = teamInfo?.presetTeamInfo?.useTeamId;
        let Isswitching = false;
        if (currentFormation === weirdTowerFormation) {
          addLog({
            time: new Date().toLocaleTimeString(),
            message: `当前已是阵容${weirdTowerFormation}，无需切换`,
            type: "info",
          });
        } else {
          await tokenStore.sendMessageWithPromise(
            tokenId,
            "presetteam_saveteam",
            { teamId: weirdTowerFormation },
            5000,
          );
          Isswitching = true;
          addLog({
            time: new Date().toLocaleTimeString(),
            message: `成功切换到阵容${weirdTowerFormation}`,
            type: "info",
          });
        }
