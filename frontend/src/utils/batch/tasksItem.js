  ...
  const batchGenieSweep = async () => {
    ...
    const taskPromises = selectedTokens.value.map(async (tokenId) => {
      ...
      try {
        ...
        for (let genieId = 1; genieId <= 4 && !shouldStop.value; genieId += 1) {
          const statKey = `genie:daily:free:${genieId}`;
          if (!isTodayAvailable(statisticsTime[statKey])) {
            addLog({
              time: new Date().toLocaleTimeString(),
              message: `${token.name} ${genieNames[genieId]}灯神今日已扫荡，跳过`,
              type: "warning",
            });
            continue;
          }
          ...
          addLog({
            time: new Date().toLocaleTimeString(),
            message: `${token.name} ${genieNames[genieId]}灯神扫荡成功`,
            type: "success",
          });
          await new Promise((r) => setTimeout(r, delayConfig.action));
        }
        ...
        for (let i = 0; i < 3 && !shouldStop.value; i += 1) {
          try {
            await tokenStore.sendMessageWithPromise(
              tokenId,
              "genie_buysweep",
            ...
            addLog({
              time: new Date().toLocaleTimeString(),
              message: `${token.name} 领取扫荡券成功 ${ticketClaimCount}/3`,
              type: "success",
            });
          } catch (err) {
            const errMsg = String(err?.message || "");
            if (errMsg.includes("购买数量超出限制") || errMsg.includes("今天已经领取过奖励了")) {
              addLog({
                time: new Date().toLocaleTimeString(),
                message: `${token.name} 今日扫荡券已领完`,
                type: "warning",
              });
              break;
            }
        ...
        addLog({
          time: new Date().toLocaleTimeString(),
          message: `${token.name} === 灯神扫荡完成（四国扫荡 ${sweepSuccessCount} 次，领取扫荡券 ${ticketClaimCount} 次）===`,
          type: "success",
        });
