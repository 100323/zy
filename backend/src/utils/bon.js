...
export class DataReader {
  ...
  get dataView() {
    return this._view || (this._view = new DataView(
      this._data.buffer,
...
export function encode(obj, enc = getEnc('x')) {
...
export function parse(buf, enc = getEnc('x')) {
  const u8 = new Uint8Array(buf);
  const plain = enc.decrypt(u8);
  if (plain[0] === 112 && plain[1] === 108) {
    const lxEnc = getEnc('lx');
    const decrypted = lxEnc.decrypt(plain);
    return bon.decode(decrypted);
  }
  if (plain[0] === 112 && plain[1] === 120) {
